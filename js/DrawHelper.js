var DrawHelper = (function() {
   ...
   var turf;
    // constructor
    function _(cesiumWidget,bufferPrimitives,aTurf) {
    ...
    // shape should aimplement setEditMode and setHighlighted
    _.prototype.registerEditableShape = function(surface) {
        ...
        setListener(surface, 'mouseMove', function(position) {
            //计算距离
            var linePos=this.positions;
            var distance=0;
            if(linePos.length>1){
                for(var i=1;i<linePos.length;i++){
                    distance=distance+Cesium.Cartesian3.distance(new Cesium.Cartesian3(linePos[i-1].x,linePos[i-1].y,linePos[i-1].z),new Cesium.Cartesian3(linePos[i].x,linePos[i].y,linePos[i].z))
                }
            }
            //计算面积
            var area;  
            var linePos=this.positions;                    
            var array=[];
            for(var i = 0, len = linePos.length; i < len; i ++){
                var cartographic = Cesium.Cartographic.fromCartesian(linePos[i]);
            ...
            if(!surface._editMode) {
                if(!surface.isPolygon){
                    _self._tooltip.showAt(position, "<p>点选编辑</p>"+"距离:"+distance/1000+"千米");
                } 
                else{                   
                    _self._tooltip.showAt(position, "<p>点选编辑</p>"+"面积:"+area+"平方米");
                }
            }
        });
...
//
    ...
    _.prototype.startDrawingMarker = function(options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            //var pickObj=scene.pick(position)
            if(position != null) {
                var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                if (cartesian) {
                    tooltip.showAt(position, "<p>点击添加标记</p>");// + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                } else {
                    tooltip.showAt(position, "<p>点击添加标记</p>");
                }
                // if(pickObj.primitive){
                //     tooltip.showAt(position, "<p>拖动改变标记位置</p>");
                // }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    ...
    _.prototype.startDrawingPolyshape = function(isPolygon, options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if(position != null) {
                if(positions.length == 0) {
                    tooltip.showAt(position, "<p>点击选择绘制起点</p>");
                } else {
                    ...
                    if (cartesian) {
                        ...
                        area=turf.area(polygons);
                        if(!isPolygon){
                            tooltip.showAt(position, "<p>点击绘制点 " + positions.length + "</p>" + (positions.length > minPoints ? "<p>双击完成绘制</p>" : "")+"长度:"+distance/1000+"千米");
                        }else{
                            tooltip.showAt(position, "<p>点击绘制点 " + positions.length + "</p>" + (positions.length > minPoints ? "<p>双击完成绘制</p>" : "")+"面积:"+area+"平方米");
                        }
            ...
            circle.setEditMode = function(editMode) {
                ...
                if(editMode) {
                    ...
                    if(this._markers == null) {
                        ...
                        var longitudeString = Cesium.Math.toDegrees(carto.longitude).toFixed(4);
                        ...
                        var handleMarkerChanges = {
                            ...
                            tooltip: function() {
                                return ("<p>拖动改变半径</p>"+"半径:"+circle.getRadius()+"</br>中心:"+"("+longitudeString+","+latitudeString+")"+"</br>共有"+n+"个建筑被选中");
                            }
                        };
    ...
    _.DrawHelperWidget = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var drawOptions = {
                ...
                extentIcon: "images/glyphicons_094_vector_path_square.png",
